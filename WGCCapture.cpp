...
{
public:
    ScreenCaptureWGC() : d3dDevice(nullptr), framePool(nullptr) {}
...
bool ScreenCaptureWGC::Initialize(HWND hwnd)
{
    ...
    if (FAILED(hr))
    {
        std::cerr << "Failed to query IDXGIDevice from ID3D11Device." << std::endl;
        return false;
    }
    ...
    // GraphicsCaptureFramePool 생성 (재사용 가능)
    framePool = Direct3D11CaptureFramePool::CreateFreeThreaded(
        CreateDirect3D11DeviceFromDXGIDevice(dxgiDevice.Get(), nullptr),
